# Shared Code Intelligence Runner

Shared code-intelligence runner logic for product repositories that want a consistent AST index without copying the same parsing, artifact, and CLI wiring into every repo.

This runner is separate from `shared-audits` because code intelligence is not a quality or security scanner.
...
`shared-code-intel/` is therefore a thin orchestration layer around external tools plus a fallback parser, not a monolithic custom indexer.
...
4. consume `shared-code-intel/` as the lower-level runtime behind the repo-local entrypoint
...
Use a thin repo-local wrapper and keep `shared-code-intel/` in the `Guidelines` repository:
...
The repository contains the same example as a standalone file in `shared-code-intel/example_code_intel.py`.
...
```bash
docker build -t guidelines-code-intel:local shared-code-intel
```

If a downstream repository wants a heavier Python and TypeScript starter profile, build the optional indexer target:

```bash
docker build --target indexers -t guidelines-code-intel:indexers shared-code-intel
```

If a downstream repository wants a separate heavy profile for Java and C#, build the JVM/.NET target:

```bash
docker build --target indexers-jvm-dotnet -t guidelines-code-intel:indexers-jvm-dotnet shared-code-intel
```

The image uses:

- `shared-code-intel/Dockerfile`
- `shared-code-intel/requirements.txt`
- `shared-code-intel/runtime/`
